...
`SmartAgriculture` 包含三部分：

- `Terminal`：运行在 ELF1 / i.MX6ULL 上的 Qt 5.6 终端，集成温湿度、光照、LED、PWM 风扇、双 RS-485、双 CAN 和 MQTT。
- `Server`：运行在 Windows/Linux 宿主机上的 Spring Boot + Moquette 服务端，提供 MQTT、REST、SSE 和监控网页。
- `PWMFanDriver`：PWM7 风扇驱动、设备树和独立 Qt 控制程序源码。

工程总说明：[SmartAgriculture/README.md](SmartAgriculture/README.md)。
纯源码 ZIP：[SmartAgriculture-source.zip](SmartAgriculture-source.zip)。
...
本仓库保留源码、构建脚本、测试代码、说明和历史测试记录。不包含预编译二进制、Qt Creator 个人 Kit 配置、缓存、日志或账号凭据；子目录说明中提到的 `bin/`、本机路径和备份文件属于此前本地部署记录，并不随本仓库提供。

智慧农业终端、宿主机服务和硬件控制链路已完成实际联调；UART2UART 的既有双向收发验证记录保留在项目文档中。
